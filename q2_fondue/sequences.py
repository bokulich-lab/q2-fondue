...
def _run_fasterq_dump(
    ...
    while retries >= 0:
        ...
        if free_space < (0.35 * used_seq_space) and not _has_enough_space(
            accession_id, tmp_dir
        ):
            LOGGER.warning(
        ...
        if result.returncode != 0:
            ...
            LOGGER.error(
                f"Fetching failed. Error: {error_msg}",
                extra={"accession_id": accession_id},
            )
    ...
    if success:
        LOGGER.info(
            "Successfully downloaded sequences", extra={"accession_id": accession_id}
        )
    else:
        LOGGER.error(
            "Failed to download sequences", extra={"accession_id": accession_id}
        )
...
def _process_downloaded_sequences(accession_id: str, output_dir: str) -> (list, list):
    ...
    filenames = [_process_one_sequence(f, output_dir) for f in filenames]
...
def _write_empty_casava(read_type: str, casava_out: str, accession_id: str):
    ...
    """
    LOGGER.warning(
        f"No {read_type}-end sequences available", extra={"accession_id": accession_id}
    )
...
def _write_to_casava(filenames: list, tmp_dir: str, casava_out: str, accession_id: str):
    ...
    if len(filenames) == 1:
        LOGGER.info(
            "Writing single-end sequences to Casava directory",
            extra={"accession_id": accession_id},
        )
        ...
        LOGGER.info(
            "Writing paired-end sequences to Casava directory",
            extra={"accession_id": accession_id},
        )
        filenames = [os.path.split(x[0])[-1] for x in sorted(filenames)]
        _copy_to_casava(filenames, tmp_dir, casava_out)
    else:
        LOGGER.error(
            "More than two files were found for the same ID while "
            "writing outputs to Casava directory",
            extra={"accession_id": accession_id},
        )
...
def _get_sequences(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if success:
            ...
            if len(single) == 0:
                _write_empty_casava("single", str(casava_out_single), accession_id)
            else:
                _write_to_casava(single, tmp_dir, str(casava_out_single), accession_id)

            # write downloaded paired-end seqs from tmp to casava dir
            if len(paired) == 0:
                _write_empty_casava("paired", str(casava_out_paired), accession_id)
            else:
                _write_to_casava(paired, tmp_dir, str(casava_out_paired), accession_id)
...
def combine_seqs(
    ...
    all_files = pd.concat(objs=[seq_artifact.manifest for seq_artifact in seqs], axis=0)
