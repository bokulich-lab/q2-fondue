...
class TestSraMetadata(TestPluginBase):
    ...
    def setUp(self):
        super().setUp()
        self.library_meta = {
            'library_name': 'fake_lib', 'library_layout': 'check',
            'library_selection': 'random', 'library_source': 'lake'
        }
        self.run_meta = {
            'public': True, 'bytes': 123, 'bases': 1236,
            'spots': 12, 'avg_spot_len': 103,
        }
        self.experiment_meta = {'instrument': 'violin', 'platform': 'illumina'}
        self.sample_meta = {'organism': 'Homo sapiens', 'tax_id': 'tax123'}
        self.study_meta = {
            'bioproject_id': 'biop123', 'center_name': 'somewhere'
        }
    ...
    def _generate_runs(self, ids, exp_id):
        return [SRARun(
            id=_id, custom_meta=self._generate_custom_meta('run'),
            experiment_id=exp_id, **self.run_meta
        ) for _id in ids]
    ...
    def _generate_exps(self, ids, smp_id, run_count=0):
        if run_count > 0:
            ...
            return [SRAExperiment(
                id=_id, custom_meta=self._generate_custom_meta('exp'),
                library=self.library, sample_id=smp_id, runs=runs[i],
                **self.experiment_meta
            ) for i, _id in enumerate(ids)]
        else:
            return [SRAExperiment(
                id=_id, custom_meta=self._generate_custom_meta('exp'),
                library=self.library, sample_id=smp_id, **self.experiment_meta
            ) for i, _id in enumerate(ids)]
    ...
    def _generate_smps(self, ids, std_id, exp_count=0):
        ...
        if exp_count > 0:
            ...
            exps = [
                self._generate_exps(
                    exp_ids[exp_count * i:exp_count * i + exp_count],
                    _id, run_count=2)
                for i, _id in enumerate(ids)
            ]
            return [SRASample(
                id=_id, **smp_meta, study_id=std_id, experiments=exps[i],
                custom_meta=self._generate_custom_meta('smp'),
            ) for i, (_id, smp_meta) in
                enumerate(self._generate_sample_meta(ids))]
        else:
            return [SRASample(
                id=_id, **smp_meta, study_id=std_id,
                custom_meta=self._generate_custom_meta('smp'),
    ...
    def test_sra_run_df(self):
        ...
        exp_df = pd.DataFrame(
            {'experiment_id': 'exp123', **self.run_meta,
    ...
    def test_sra_experiment_df_no_run(self):
        ...
        exp_df = pd.DataFrame(
            {'sample_id': 'smp123', **self.experiment_meta,
             **self._generate_custom_meta('exp'), **self.library_meta},
    ...
    def test_sra_experiment_with_runs(self):
        ...
        exp_df = pd.DataFrame(
            [{'sample_id': 'smp123', 'experiment_id': 'exp123',
              **self.experiment_meta, **self._generate_custom_meta('exp'),
              **self._generate_custom_meta('run'), **self.run_meta,
              **self.library_meta}] * len(exp_run_ids),
    ...
    def test_sra_sample_no_exp(self):
        ...
        exp_df = pd.DataFrame(
             ...
             **self._generate_custom_meta('smp'), 'study_id': 'std123'},
    ...
    def test_sra_sample_with_exps(self):
        ...
        exp_df = pd.DataFrame(
            [{'sample_id': 'smp123', 'experiment_id': exp_id,
              'study_id': 'std123', **self.experiment_meta,
              **self.run_meta, **self.library_meta, **self.sample_meta,
              **self._generate_sample_meta(['smp123'])[0][1],
    ...
    def test_sra_study_with_smps(self):
        ...
        exp_df = pd.DataFrame(
            [{'sample_id': samp_id, 'experiment_id': exp_id,
              'study_id': 'std123', **self.experiment_meta, **samp_meta,
              **self.run_meta, **self.library_meta, **self.sample_meta,
              **self.study_meta, **self._generate_custom_meta('std'),
              **self._generate_custom_meta('smp'),
              **self._generate_custom_meta('exp'),
