...
from q2_fondue.scraper import (
    ...
    _find_accession_ids,
    _find_doi_in_extra,
    _find_doi_in_arxiv_url,
    _get_parent_and_doi,
    _expand_dict,
    _link_attach_and_doi,
    _get_attachment_keys,
    scrape_collection,
    NoAccessionIDs,
)
...
class TestUtils4CollectionScraping(TestPluginBase):
    ...
    def test_get_correct_collection_id(self, patch_col, patch_ever):
        patch_ever.return_value = self._open_json_file("scraper_collections.json")
    ...
    def test_get_collection_id_raiseError(self, patch_col, patch_ever):
        ...
        col_name = "BlaBla"
        with self.assertRaisesRegex(KeyError, f"name {col_name} does not exist"):
    ...
    def test_find_special_id_two_matches(self):
        txt = "PRJDB1234: 2345 and 4567. How about another study? " "PRJEA9876: 8765."
        pattern = r"PRJ[EDN][A-Z]\s?\d+:\s\d+"
    ...
    def test_find_accession_ids_no_double(self):
        txt_w_2ids = (
            "this data available in PRJEB4519 and PRJEB4519. Also "
    ...
    @parameterized.expand(
        ["SRP1 32205", "SRP13 2205", "SRP132 205", "SRP1322 05", "SRP13220 5"]
    )
    ...
    def test_find_accession_ids_special_cases_one_comma(self):
        ...
        txt_diff = "under accession numbers PRJEB11895, 12577 and " "41427"
    ...
    def test_find_accession_ids_special_cases_three_commas(self):
        ...
        # https://doi.org/10.1038/s41467-021-26215-w
        txt_diff = (
            "under accession numbers PRJEB11895, 12577, 34555, " "89765 and 41427"
        )
        exp_proj = [
            "PRJEB11895",
            "PRJEB12577",
    ...
    def test_find_accession_ids_special_cases_hyphen_three_digits_over_hundred(self):
    ...
    def test_find_accession_ids_special_cases_hyphen_accession_over_hundred(self):
    ...
    def test_find_accession_ids_special_cases_no_hyphen(self):
        # example inspired by this publication:
    ...
    def test_expand_dict_new_items(self):
        ...
        exp_out = {"accID1": ["doi1"], "accID2": ["new_doi"], "accID3": ["new_doi"]}
    ...
    def test_expand_dict_existing_item_extend(self):
        ...
        exp_out = {"accID1": ["doi1", "new_doi"], "accID3": ["new_doi"]}
    ...
    def test_expand_dict_no_duplicate_dois(self):
        ...
        exp_out = {"accID1": ["doi1"], "accID3": ["doi1"]}
...
@patch.dict(
    os.environ,
    {"ZOTERO_TYPE": "user", "ZOTERO_USERID": "12345", "ZOTERO_APIKEY": "myuserkey"},
)
class TestCollectionScraping(TestUtils4CollectionScraping):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.all_id_types = ["run", "study", "bioproject", "experiment", "sample"]
    ...
    def test_collection_scraper_all_ids(
        self, patch_zot_txt, patch_col, patch_items, patch_get_col_id
    ):
        ...
        patch_zot_txt.return_value = {
            "content": "This is full-text with PRJEB4519, ERP123456, ERR2765209, "
    ...
    def test_collection_scraper_one_id_type(
        self, id_type, acc_id, patch_zot_txt, patch_col, patch_items, patch_get_col_id
    ):
        # define patched outputs
        patch_items.return_value = self._open_json_file(
            "scraper_items_journalarticle.json"
    ...
    def test_collection_scraper_no_ids(
        ...
        with self.assertRaisesRegex(NoAccessionIDs, f"collection {col_name} does not"):
    ...
    def test_collection_scraper_nofulltext(
        ...
        patch_zot_txt.side_effect = [
            zotero_errors.ResourceNotFoundError,
            {
    ...
    def test_collection_scraper_no_doi_ignore(
        ...
        # define patched outputs
        patch_items.return_value = self._open_json_file("scraper_items_no_doi.json")
        patch_zot_txt.side_effect = [
            zotero_errors.ResourceNotFoundError,
            {
                "content": "This is full-text with PRJEB4519.",
        ...
        exp_out = self._create_exp_out({"bioproject": {"PRJEB4519": [""]}})
    ...
    def test_collection_scraper_multiple_dois(
        ...
        patch_zot_txt.side_effect = [
            ...
            {"content": "IDs are in PRJEB4519.", "indexedPages": 50, "totalPages": 50},
        ]
