...
from q2_fondue.utils import _validate_run_ids, _determine_id_type
...
class FakeAnalyzerValidation:
    def __init__(self, ids, counts):
        ...
        self.result.result = pd.Series(data=counts, index=ids)
...
class TestMetadataFetching(_TestPluginWithEntrezFakeComponents):
    ...
    def generate_meta_df(self, obs_suffices, exp_suffix):
        ...
        for s in obs_suffices:
            meta_dfs.append(
                pd.read_csv(
                    self.get_data_path(f"sra-metadata-{s}.tsv"), sep="\t", index_col=0
                )
            )
        exp_df = pd.read_csv(
            self.get_data_path(f"sra-metadata-{exp_suffix}.tsv"), sep="\t", index_col=0
        )
    ...
    def test_efetcher_inquire_single(self):
        with patch.object(Requester, "request") as mock_request:
            ...
            obs_df, _ = _efetcher_inquire(self.fake_efetcher, ["FAKEID1"], "INFO")
        ...
        for arg in self.efetch_request_properties:
            self.assertEqual(getattr(exp_request, arg), getattr(obs_request, arg))
    ...
    def test_efetcher_inquire_single_multisample(self):
        with patch.object(Requester, "request") as mock_request:
            mock_request.return_value = self.xml_to_response("single", "_multisample")
            obs_df, _ = _efetcher_inquire(self.fake_efetcher, ["FAKEID1"], "INFO")
        (obs_request,) = mock_request.call_args.args
        exp_request = self.generate_ef_request(["FAKEID1"])
        exp_df = self.generate_expected_df().iloc[[0]]
    ...
    def test_efetcher_inquire_multi(self):
        ...
        (obs_request,) = mock_request.call_args.args
        exp_request = self.generate_ef_request(["FAKEID1", "FAKEID2"], size=2)
    ...
    def test_execute_efetcher(self, patch_efetch_iq, patch_ef):
        ...
        _, _ = _execute_efetcher(
            "someone@somewhere.com", 1, ["Valid1", "Valid2", "Valid3"], "INFO"
        )
    ...
    def test_efetcher_inquire_error(self):
        with patch.object(Requester, "request") as mock_response:
            ...
            exp_missing = {"FAKEID1": '<?xml version="1.0"  ?>\n<ERROR>\n</ERROR>\n'}
    ...
    def test_esearcher_inquire_multi(self, mock_search):
        with patch.object(Requester, "request") as mock_request:
            mock_search.return_value = self.fake_esearcher
            mock_request.return_value = self.json_to_response("multi", "_correct", True)
            obs_result = _validate_run_ids(
        ...
        (obs_request,) = mock_request.call_args.args
        exp_request = self.generate_es_request("SRR000001 OR SRR000013 OR ERR3978173")
    ...
    def test_validate_run_ids_one_batch(self, mock_analyzer, mock_search):
        ids = ["SRR000001", "SRR000013", "ERR3978173"]
        mock_analyzer.return_value = FakeAnalyzerValidation(ids, [1, 1, 1])
        mock_search.return_value.inquire = mock_analyzer

        obs_invalid = _validate_run_ids("someone@somewhere.com", 1, ids, "INFO")

        self.assertEqual(
            mock_analyzer.return_value.result.validate_result.call_count, 1
        )
    ...
    def test_validate_run_ids_multiple_batches(
        ...
        ids = ["SRR000001", "SRR000013", "ERR3978173"]
        mock_chunker.return_value = (["SRR000001", "SRR000013"], ["ERR3978173"])

        first_analyzer = FakeAnalyzerValidation(["SRR000001", "SRR000013"], [1, 1])
        second_analyzer = FakeAnalyzerValidation(["ERR3978173"], [1])
        mock_analyzer.side_effect = [first_analyzer, second_analyzer]
        mock_search.return_value.inquire = mock_analyzer

        obs_invalid = _validate_run_ids("someone@somewhere.com", 1, ids, "INFO")

        self.assertEqual(first_analyzer.result.validate_result.call_count, 1)
        self.assertEqual(second_analyzer.result.validate_result.call_count, 1)
        self.assertDictEqual(obs_invalid, {})
    ...
    def test_find_all_run_ids_big_runset(self):
        fake_results = [
            {
                "RUN_SET": [
                    {"RUN": {"@accession": "ab12", "@alias": "run123"}},
    ...
    def test_get_run_meta(self, patch_ef, patch_val):
        exp_df = pd.DataFrame(
            {"meta1": [1, 2, 3], "meta2": ["a", "b", "c"]}, index=["AB", "cd", "Ef"]
        )
    ...
    def test_get_run_meta_missing_ids(self, patch_ef, patch_val):
        exp_meta = pd.DataFrame(
            {"meta1": [1, 2], "meta2": ["a", "b"]}, index=["AB", "cd"]
        )
    ...
    def test_get_run_meta_one_invalid_id(self, patch_ef, patch_val, patch_es):
        patch_val.return_value = {"AB": "ID is invalid."}
        exp_df = pd.DataFrame({"meta1": [1], "meta2": ["a"]}, index=["AA"])
        patch_ef.return_value = (exp_df, [])
    ...
    def test_get_other_meta_different_ids(
        self, id_type, db2search, mock_analyzer, mock_search, mock_get
    ):
        exp_ids = [
            "SRR000007",
            "SRR000018",
        ...
        mock_analyzer.return_value = MagicMock(result=MagicMock(uids=fake_uids))
        mock_search.return_value.inquire = mock_analyzer
        with patch.object(conduit, "Conduit") as mock_conduit:
            ...
            obs_ids = _get_other_meta(
                "someone@somewhere.com", 1, ["AB", "cd"], id_type, "INFO", MagicMock()
            )
    ...
    def test_get_metadata_run(self, patched_get_other, patched_get_run):
        ...
        patched_get_run.assert_called_once_with(
            "abc@def.com", 2, ["SRR123", "SRR234", "SRR345"], False, "INFO", ANY
        )
    ...
    def test_get_metadata_other(
        ...
        patched_get_other.assert_called_once_with(
            "abc@def.com", 2, acc_ids, id_type, "INFO", ANY
        )
        patched_get_run.assert_not_called()

    @parameterized.expand(["run", "study", "bioproject", "experiment", "sample"])
    def test_find_doi_mapping_and_type(self, id_type):
        map_ids_doi = Metadata.load(self.get_data_path(f"{id_type}_ids_w_doi.tsv"))
    ...
    @patch("q2_fondue.metadata._get_run_meta")
    @patch("q2_fondue.metadata._get_other_meta")
    def test_get_metadata_run_w_doi(self, patched_get_other_study, patched_get_run):
        meta_df = pd.read_csv(
            self.get_data_path("sra-metadata-1.tsv"), sep="\t", index_col=0
        )
    ...
    def test_get_metadata_other_w_doi(
        ...
        ids_meta = Metadata.load(self.get_data_path(f"{id_type}_ids_w_doi.tsv"))
    ...
    def test_get_metadata_missing_ids_refetch_w_doi(
        ...
        meta_df = pd.read_csv(
            self.get_data_path("sra-metadata-failed-ids.tsv"), sep="\t", index_col=0
        )
        patched_get_run.return_value = (meta_df, {})

        failed_ids = Metadata.load(self.get_data_path("failed_ids_no_doi.tsv"))
        ids_doi = Metadata.load(self.get_data_path(f"{id_type}_ids_w_doi.tsv"))

        obs_meta, _ = get_metadata(failed_ids, "abc@def.com", linked_doi=ids_doi)
        self.assertTrue("DOI" in obs_meta.columns)
        if id_type == "run":
